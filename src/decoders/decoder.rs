...
pub fn check_objects<'a>(
    format_string: &'a str,
    message_values: &'a [FirehoseItemInfo],
    item_type: u8,
    item_index: usize,
) -> Cow<'a, str> {
    ...
    if item_type == precision_item {
        ...
        if index > message_values.len() {
            return format!("Index out of bounds for FirehoseItemInfo Vec. Got adjusted index {}, Vec size is {}. This should not have happened", index, message_values.len()).into();
        }
    }
    ...
    {
        return message_values[index].message_strings.as_str().into();
    }

    // Check if log value contains one the supported decoders
    if format_string.contains("BOOL") {
        uppercase_bool(&message_values[index].message_strings).into()
    } else if format_string.contains("bool") {
        lowercase_bool(&message_values[index].message_strings).into()
        ...
        errors(&message_values[index].message_strings).into()
    } else if format_string.contains("odtypes:mbridtype") {
        member_id_type(&message_values[index].message_strings).into()
        ...
        client_authorization_status(&message_values[index].message_strings).into()
    } else if format_string.contains("location:CLDaemonStatus_Type::Reachability") {
        daemon_status_type(&message_values[index].message_strings).into()
    } else if format_string.contains("location:CLSubHarvesterIdentifier") {
        subharvester_identifier(&message_values[index].message_strings).into()
        ...
        io_message(&message_values[index].message_strings).into()
        ...
        dns_addrmv(&message_values[index].message_strings).into()
    } else if format_string.contains("mdns:rrtype") {
        dns_records(&message_values[index].message_strings).into()
    } else if format_string.contains("mdns:nreason") {
        dns_reason(&message_values[index].message_strings).into()
    } else if format_string.contains("mdns:protocol") {
        dns_protocol(&message_values[index].message_strings).into()
        ...
        dns_yes_no(&message_values[index].message_strings).into()
    } else if format_string.contains("mdns:acceptable") {
        dns_acceptable(&message_values[index].message_strings).into()
    } else if format_string.contains("mdns:gaiopts") {
        dns_getaddrinfo_opts(&message_values[index].message_strings).into()
    } else {
        "".into()
    }
}
...
mod tests {
    ...
    fn test_check_objects_lowercase_bool() {
        ...
        let message_values = vec![test_item_info];
        let results = check_objects(test_format, &message_values, test_type, test_index);
